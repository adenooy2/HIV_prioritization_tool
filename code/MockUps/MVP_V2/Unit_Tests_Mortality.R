...
# ============================================================================
# TEST 2: UNDIAGNOSED GROUP MORTALITY RATE
# ============================================================================

test_that("Undiagnosed group deaths use untreated_undiagnosed rate with AHD blend", {
  cat("\n========================================\n")
  cat("TEST 2: Undiagnosed Group Mortality\n")
  cat("========================================\n")

  set_mortality(untreated = 0.10, ahd = 0.20,
                prop_ahd_undiag = 0.20, prop_ahd_diag_not_art = 0.00)
  ints <- zero_interventions()
  out  <- calculate_scenario_outcomes(ctx, ints, pops)
  ...
  expect_equal(out$end_deaths, round(expected_deaths_undiag),
               info = "Undiagnosed deaths = n * (0.8*base + 0.2*ahd)")
...
# ============================================================================
# TEST 3: DIAGNOSED-NOT-ON-ART GROUP USES SAME RATE AS UNDIAGNOSED
# ============================================================================

test_that("Diagnosed-not-on-ART group uses untreated_undiagnosed rate", {
  cat("\n========================================\n")
  cat("TEST 3: Diagnosed-Not-on-ART Mortality\n")
  cat("========================================\n")

  set_mortality(untreated = 0.10, ahd = 0.20,
                prop_ahd_undiag = 0.00, prop_ahd_diag_not_art = 0.20)
  ints <- zero_interventions()
  out  <- calculate_scenario_outcomes(ctx, ints, pops)

  expected <- calc_deaths_expected(n_diag_not_art, 0.10, 0.20, 0.20)
  cat(sprintf("  Expected deaths (diagnosed not ART): %.1f\n", expected))
  cat(sprintf("  end_deaths: %g\n", out$end_deaths))

  expect_equal(out$end_deaths, round(expected),
               info = "Diagnosed-not-ART deaths use untreated_undiagnosed rate")
...
test_that("Established treated group uses treated rate (prop_ahd = 0)", {
  ...
  set_mortality(treated = 0.008, ahd = 0.20,
                prop_ahd_est_treated = 0.00)  # production value
  ints <- zero_interventions()
  out  <- calculate_scenario_outcomes(ctx, ints, pops)
...
test_that("Established suppressed group uses suppressed rate (prop_ahd = 0)", {
  ...
  set_mortality(suppressed = 0.003, ahd = 0.20,
...
test_that("prop_ahd=0 for established groups: AHD rate changes have no effect", {
  ...
  set_mortality(treated=0.008, suppressed=0.003, ahd=0.20,
                prop_ahd_est_treated=0, prop_ahd_est_supp=0)
  ints <- zero_interventions()
  out_low_ahd <- calculate_scenario_outcomes(ctx, ints, pops)

  set_mortality(treated=0.008, suppressed=0.003, ahd=0.99,  # extreme AHD rate
                prop_ahd_est_treated=0, prop_ahd_est_supp=0)
...
test_that("Cotrimoxazole reduces base mortality rate for new initiations", {
  ...
  expect_lt(out_with$end_deaths, out_no$end_deaths,
            info = "Cotrimoxazole should reduce deaths")
  expect_equal(out_with$deaths_averted,
               round(art_init * 0.80 * 0.10 * 0.50),
               info = "Deaths averted = coverage × efficacy × unadjusted deaths")
...
test_that("OI management reduces base mortality rate for new initiations", {
  ...
  expect_lt(out_with$end_deaths, out_no$end_deaths,
            info = "OI management should reduce deaths")
...
test_that("Cotrimoxazole and OI management effects are multiplicative", {
  ...
  ints_both$test_facility_general <- 50000
  ints_both$cotrimoxazole <- 100
  ints_both$oi_management <- 100

  out_none <- zero_interventions()
  out_none$test_facility_general <- 50000
  out_none <- calculate_scenario_outcomes(ctx, out_none, pops)

  out_both <- calculate_scenario_outcomes(ctx, ints_both, pops)

  art_init <- out_none$art_initiations

  # eff_base = 0.10 * (1-0.50) * (1-0.40) = 0.10 * 0.50 * 0.60 = 0.03
  expected_both <- round(art_init * 0.80 * 0.03)

  cat(sprintf("  art_initiations: %g\n", art_init))
  cat(sprintf("  Expected deaths (both): %g\n", expected_both))
  cat(sprintf("  Actual  deaths (both):  %g\n", out_both$end_deaths))

  expect_equal(out_both$end_deaths, expected_both,
               info = "Combined effect is multiplicative: base * (1-cotrix) * (1-OI)")
...
test_that("AHD package with CD4 testing reduces AHD mortality for new initiations", {
  ...
  # Only AHD rate matters (zero base rate)
  ...
  expect_lt(out_with$end_deaths, out_base$end_deaths,
            info = "AHD package + CD4 testing reduces deaths")
...
test_that("AHD package effect scales proportionally with CD4 coverage", {
  ...
  expect_lt(out_cd4_50$deaths_averted, out_cd4_100$deaths_averted,
            info = "Higher CD4 coverage → more deaths averted")
...
test_that("More testing produces more ART initiations and more deaths averted", {
  ...
  expect_gt(out_high$art_initiations, out_low$art_initiations,
            info = "More testing → more ART initiations")
  expect_gt(out_high$deaths_averted, out_low$deaths_averted,
            info = "More ART initiations → more deaths averted from mortality interventions")
...
cat("✓ TEST 2:  Undiagnosed group mortality rate\n")
cat("✓ TEST 3:  Diagnosed-not-on-ART mortality rate\n")
cat("✓ TEST 4:  Established treated group mortality\n")
cat("✓ TEST 5:  Established suppressed group mortality\n")
